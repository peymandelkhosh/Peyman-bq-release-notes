...
document.addEventListener('DOMContentLoaded', () => {
    ...
    refreshBtn.addEventListener('click', () => {
        ...
        // Clear search input on refresh so user sees entire fresh list
        searchInput.value = '';
        fetchNotes().finally(() => {
            setTimeout(() => {
...
function renderNotes(notes) {
    ...
    if (notes.length === 0) {
        container.innerHTML = `
            <div class="no-results">
                <p>No release notes match your search query.</p>
            </div>
        `;
        return;
    }
    ...
    notes.forEach((note, index) => {
        ...
        card.style.animationDelay = `${index * 0.03}s`; // Slightly faster animations for dynamic filtering
...
function filterNotes(query) {
    ...
    const filtered = currentNotes.filter(note => {
        ...
        const plainContentText = htmlToPlainText(note.content).toLowerCase();
        return titleText.includes(term) || plainContentText.includes(term);
    });
